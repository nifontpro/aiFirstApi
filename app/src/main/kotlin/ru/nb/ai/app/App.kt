...
fun main() {
    ...
    val baseUrl = config.getProperty("litellm.baseUrl")
    val apiKey = config.getProperty("litellm.apiKey")
    val timeoutSeconds = config.getProperty("litellm.timeoutSeconds", "300").toLongOrNull() ?: 300
    ...
    val prompt = AttributedString("> ", AttributedStyle.DEFAULT.foreground(AttributedStyle.GREEN))
        .toAnsi(terminal)
        ...
        while (true) {
            ...
            when {
                ...
                input.startsWith("/system", ignoreCase = true) -> {
                    val arg = input.drop("/system".length).trim()
                ...
                input.equals("/models", ignoreCase = true) -> {
                    val newModel = ModelSelector.select(AVAILABLE_MODELS, model, terminal)
                    if (newModel != model) {
                        model = newModel
                        rebuildClient()
                        history.clear()
                ...
                input.startsWith("/think", ignoreCase = true) -> {
                    val arg = input.drop("/think".length).trim()
                    showThinking = when {
                ...
                input.startsWith("/t", ignoreCase = true) -> {
                    val arg = input.drop("/t".length).trim()
                ...
                input.startsWith("/request", ignoreCase = true) -> {
                    val arg = input.drop("/request".length).trim()
                    showRequest = when {
                        arg.equals("on", ignoreCase = true)  -> true
                        arg.equals("off", ignoreCase = true) -> false
                        else -> {
